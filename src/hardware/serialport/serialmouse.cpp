...
void CSerialMouse::SetType(const MouseType type)
{
    if (type != mouse_type) {
        mouse_type = type;
...
void CSerialMouse::OnMouseEventMoved(const int16_t delta_x, const int16_t delta_y)
{
...
void CSerialMouse::OnMouseEventButton(const uint8_t buttons, const uint8_t idx)
{
...
void CSerialMouse::OnMouseEventWheel(const int8_t delta_w)
{
...
void CSerialMouse::StartPacketData(const bool extended)
{
    ...
    if (mouse_type == MouseType::MICROSOFT ||
        ...
        uint8_t dx = ClampDelta(mouse_delta_x);
        uint8_t dy = ClampDelta(mouse_delta_y);
        ...
        packet[1]  = ClampDelta(mouse_delta_x);
        packet[2]  = ClampDelta(-mouse_delta_y);
        packet_len = 3;
...
void CSerialMouse::StartPacketPart2()
{
    ...
    if (mouse_type == MouseType::MOUSE_SYSTEMS) {
        ...
        packet[0] = ClampDelta(mouse_delta_x);
        packet[1] = ClampDelta(-mouse_delta_y);
