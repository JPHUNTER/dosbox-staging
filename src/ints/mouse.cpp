...
static void AddEvent(const uint8_t type) {
    if (queue_used < QUEUE_SIZE) {
        if (queue_used > 0) {
            /* Skip duplicate events */
            if (type==EventType::MOUSE_HAS_MOVED || type==EventType::WHEEL_HAS_MOVED) return;
...
static void CursorMoved(float xrel,float yrel,float x,float y,bool emulate) {
    ...
    AddEvent(EventType::MOUSE_HAS_MOVED);
    DrawCursor();
}

static uint8_t GetResetWheel8bit() {
    if (!mouse.cute_mouse) return 0;
    const int8_t tmp = static_cast<int8_t>(std::clamp(mouse.wheel, static_cast<int16_t>(INT8_MIN), static_cast<int16_t>(INT8_MAX)));
    mouse.wheel = 0;
...
static Bitu INT33_Handler(void) {
    ...
    case 0x05: // MS MOUSE v1.0+ / CuteMouse - return button press data / mouse wheel data
        {
            uint16_t but = reg_bx;
            if (but == 0xffff && mouse.cute_mouse) { // 'magic' index for checking wheel instead of button
...
void MOUSE_EventWheel(const int32_t w_rel) {
    ...
    if (mouse.cute_mouse) {
        mouse.wheel = std::clamp(w_rel + mouse.wheel, INT16_MIN, INT16_MAX);
        mouse.last_wheel_moved_x = POS_X;
        mouse.last_wheel_moved_y = POS_Y;

        AddEvent(EventType::WHEEL_HAS_MOVED);
    }
}
